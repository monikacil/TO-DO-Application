...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function changeElementClass(element, className, classIndex){
        element.classList.remove(className + "-" + (classIndex-1)%3);
        element.classList.remove(className + "-" + (classIndex+1)%3);
        element.classList.add(className + "-" + classIndex%3);
    }

    function changeElementsClass(array, className, classIndex){
        for(var i = 0; i < array.length; i++){
            changeElementClass(array[i], className, classIndex);
        }
    }
    ...
    function changeButtonsColor(classIndex){
        ...
        changeElementsClass(fas, 'new-border-color', classIndex);
        changeElementsClass(sortType, 'new-color', classIndex);
        changeElementsClass(optionsChangeStyle, 'new-color', classIndex);

        changeElementClass(h1, 'new-color', classIndex);
        changeElementClass(changeStyle, 'new-color', classIndex);
        changeElementClass(todolist, 'new-main-border', classIndex);
    }
